...
class CartPendulumEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    The CartPendulum is placed upright on the cart and the goal is to balance the pole by applying forces
     in the left and right direction on the cart.

    ## Action Space

    The action is a `ndarray` with shape `(1,)` which can take values `[-1.0, 1.0]` indicating the direction
     of the fixed force the cart is pushed with.

    - -1: Push cart to the left with maximum power
    - 0: Do nothing
    - 1: Push cart to the right with maximum power
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.force_mag = 10.0 * 2
        self.tau = 0.02  # seconds between state updates
        self.kinematics_integrator = "euler"

        # Angle at which to fail the episode
        self.theta_threshold_radians = 12 * 2 * math.pi / 360
        self.x_threshold = 2.4 * 1.5
        ...
        self.action_space = spaces.Box(low=np.array([-self.force_mag]), high=np.array([self.force_mag]), dtype=np.float32)
    ...
    def step(self, force):
        ...
        force = max(-1.0, min(1.0, force)) * self.force_mag
        # print('force', force)
        ...
        if terminated:
            print('terminate', x, self.x_threshold)
...
class CartPendulumVectorEnv(VectorEnv):
    metadata = {
        "render_modes": ["human", "rgb_array"],
        "render_fps": 50,
    }
    ...
    def __init__(
        ...
        self.single_action_space = spaces.Box(low=np.array([-self.force_mag]), high=np.array([self.force_mag]), dtype=np.float32)
